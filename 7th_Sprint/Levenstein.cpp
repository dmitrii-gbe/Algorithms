...
целых чисел, из которых buffer предназначен для хранения уже рассчитанных расстояний между префиксом
фиксированной длины N одной из строк и всеми префиксами второй строки. Во втором массиве active сохраняются
результаты текущих рассчетов, для которых испоьзуется информация из buffer. При этом имеется ввиду, что
...
(по диагонали) от вычисляемого. По оканчании вычисления элементов массива active, содержимое active и buffer
